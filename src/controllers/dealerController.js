...
export const createDealer = async (req, res) => {
  try {
    ...
    const dealer = await prisma.dealer.create({ data: { name, email, phone, price } });
    res.status(201).json({ message: 'Dealer created', data: dealer });
  } catch (err) {
...
export const updateDealer = async (req, res) => {
  try {
    ...
    const updated = await prisma.dealer.update({ where: { id }, data: { name, email, phone, price } });
    res.status(200).json({ message: 'Dealer updated', data: updated });
  } catch (err) {
